...
def _required_channel_username() -> str:
    normalized = _normalize_required_channel(REQUIRED_CHANNEL)
    return normalized or REQUIRED_CHANNEL


def _resolve_user_join_status(user_id: str, *, force_refresh: bool = False) -> dict:
    uid = str(user_id)
    if force_refresh:
        invalidate_join_cache_entry(uid)
    try:
        if database_ready():
            with get_db_session() as db:
                return resolve_channel_membership(
                    uid,
                    _check_channel_membership,
                    db=db,
        ...
        print(f"⚠️ join status error: {exc}")
...
def _build_start_reply_markup(joined: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(text="✅ تایید عضویت", callback_data="verify_join")],
    ])


def _start_welcome_text(joined: bool) -> str:
    if joined:
        return (
            "به Amir BTC Assistant خوش آمدی!\n\n"
            "عضویت شما تایید شده است. برای ورود، دکمه زیر را بزنید."
        )
    channel = _required_channel_username()
    return (
        "به Amir BTC Assistant خوش آمدی!\n\n"
        f"برای استفاده از مینی‌اپ، ابتدا در کانال @{channel} عضو شوید.\n"
        "بعد از عضویت، دکمه «تایید عضویت» را بزنید."
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not update.message:
        return
    result = _resolve_user_join_status(str(user.id))
    joined = bool(result.get("joined"))
    await update.message.reply_text(
        _start_welcome_text(joined),
...
async def verify_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if joined:
        ...
        text = (
            ...
            f"لطفاً ابتدا در کانال @{channel} عضو شوید، سپس دوباره «تایید عضویت» را بزنید."
        )
    try:
