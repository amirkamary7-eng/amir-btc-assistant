...
import { createChannelMembershipService } from './src/services/channel-membership.js';
...
// ─── Channel Membership functions moved to src/services/channel-membership.js (7 functions) ───
// Extracted: getChatMemberDebugPayload, checkChannelMembership,
//   _getActiveAdChannels, _hashChannelSet, _checkSingleTelegramChannel,
//   checkAdditionalRequiredChannels, resolveChannelMembership
//
// All 7 functions are returned from createChannelMembershipService factory
// (initialized in the composition root below, after createReferralRewardsService
// so processPendingReferralReward is in scope, and before createMembershipGateway
// so the channel-membership helpers are available as DI to the gateway).
//
// Kept in worker-proxy.js (core helpers / cycle-breakers):
//   - JOINED_STATUSES constant (line ~1143) — used by isJoinedMember
//   - isJoinedMember function (line ~1167) — DI'd into the channel-membership factory
//   - resolveRequiredChannel, getTelegramChatId, isBotConfigured, isAdminTelegramId,
//     isDatabaseConfigured, safeError — DI'd into the factory
//   - getCachedJoinStatus, setCachedJoinStatus, _kvWriteDedup, getDbUserJoinState,
//     persistDbUserJoinState — join-state infrastructure, DI'd into the factory
//   - advertisementsRepo (composition root, line ~4456) — accessed via
//     getAdvertisementsRepo lazy getter (DI #14) to avoid TDZ cycle (factory
//     initializes BEFORE advertisementsRepo const is declared)
//   - processPendingReferralReward — from createReferralRewardsService, DI'd as
//     cycle-breaker
//
...
// Placed here (after createReferralRewardsService) — processPendingReferralReward (DI #13) is in
// scope as a cycle-breaker from the referral-rewards factory above. Placed BEFORE
// createMembershipGateway (line ~3887) so checkChannelMembership + checkAdditionalRequiredChannels
// are in scope for the gateway DI. Placed BEFORE createUserHandlers (line ~3898) so
// resolveChannelMembership is in scope for userHandlers DI pass-through.
//
// TDZ NOTE: advertisementsRepo is a `const` initialized at line ~4090 (AFTER this
// factory call). To preserve the original late-binding pattern
// (`typeof advertisementsRepo === 'undefined'`) without creating a bare reference
// in the extracted module, we pass a getter closure as DI #14 (getAdvertisementsRepo).
// The getter returns `advertisementsRepo` at runtime — always defined by the time
// any request handler runs (well after module-init completes).
const {
...
} = createChannelMembershipService({
  setCachedJoinStatus,
  getCachedJoinStatus,
  isDatabaseConfigured,
  isBotConfigured,
  ...
  safeError,
  processPendingReferralReward,
  getAdvertisementsRepo: () => (typeof advertisementsRepo !== 'undefined' ? advertisementsRepo : undefined),
});
