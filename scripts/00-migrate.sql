...
-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION: Bilingual Content (FA/EN) — app_content + membership_rules
-- ═══════════════════════════════════════════════════════════════════════════
-- Adds English columns to two tables that previously stored Persian-only
-- content, and backfills English seed content for existing rows.
--
-- SAFETY (matches the file-level hard constraints above):
--   * Every ADD COLUMN uses IF NOT EXISTS — safe on existing tables.
--   * Every backfill UPDATE is gated by WHERE <col>_en IS NULL — admin edits
--     to English content are preserved on re-runs.
--   * NO DROP, NO DELETE, NO TRUNCATE, NO modification of existing Persian
--     columns (title, sections, body_markdown, summary) anywhere in this
--     section. Persian content is never touched.
--   * All statements run inside the surrounding BEGIN/COMMIT transaction so
--     a partial failure rolls back cleanly.
--
-- Source of truth: scripts/bilingual-content-schema.sql (kept as the
-- standalone reference file; this section mirrors its DDL + backfills so
-- the production deploy workflow runs them via `psql -f scripts/00-migrate.sql`).
-- ═══════════════════════════════════════════════════════════════════════════

-- ── app_content: add English columns ──────────────────────────────────────
...
-- ── app_content: backfill English content for existing rows ────────────────
...
-- ── membership_rules: add English columns ─────────────────────────────────
...
-- ── membership_rules: backfill English content for existing rows ──────────
-- Only fills rows where title_en IS NULL (preserves admin edits on re-run).
