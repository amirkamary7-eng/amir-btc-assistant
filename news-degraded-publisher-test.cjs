...
test('DEGRADED-001: DEGRADED_PUBLISHERS set contains www.coindesk.com', () => {
  assert.ok(SUMMARY_SRC.includes('DEGRADED_PUBLISHERS'),
...
test('DEGRADED-002: DEGRADED_PUBLISHERS checked before article fetch (STEP 0.5 before STEP 1)', () => {
  const step05Idx = SUMMARY_SRC.indexOf('STEP 0.5: Check DEGRADED_PUBLISHERS');
...
test('DEGRADED-003: Degraded publisher uses RSS description (stripTags + title + length check)', () => {
  const degradedBlock = SUMMARY_SRC.slice(
    SUMMARY_SRC.indexOf('DEGRADED_PUBLISHERS.has(articleHostname)'),
    SUMMARY_SRC.indexOf('DEGRADED_PUBLISHERS.has(articleHostname)') + 800
  );
...
test('DEGRADED-004: Degraded publisher with too-short RSS → clean skip (failed, not retried)', () => {
  ...
  const degradedCheckIdx = SUMMARY_SRC.indexOf('DEGRADED_PUBLISHERS.has(articleHostname)');
  const degradedBlock = SUMMARY_SRC.slice(degradedCheckIdx, degradedCheckIdx + 1500);
...
test('DEGRADED-005: Non-degraded publisher (Cointelegraph) → article fetch proceeds normally', () => {
  ...
  const fetchSection = SUMMARY_SRC.slice(step1Idx, step1Idx + 2000);
...
test('DEGRADED-006: 403 fallback still works for non-degraded publishers', () => {
  ...
  const status403Idx = SUMMARY_SRC.indexOf('articleRes.status === 403');
  assert.ok(status403Idx > -1, '403 status check must exist');
  const fetchSection = SUMMARY_SRC.slice(status403Idx, status403Idx + 1000);
...
test('DEGRADED-007: 429 Retry-After still works for non-degraded publishers', () => {
  const retrySection = SUMMARY_SRC.slice(
    SUMMARY_SRC.indexOf('articleRes.status === 429'),
    SUMMARY_SRC.indexOf('articleRes.status === 429') + 500
  );
...
test('DEGRADED-009: SSRF protection intact (URL scheme validation)', () => {
  ...
  assert.ok(SUMMARY_SRC.includes("invalid_url_scheme"),
    'URL scheme validation must still exist');
  assert.ok(SUMMARY_SRC.includes("/^https?:\\/\\//i.test(article.url)"),
...
test('DEGRADED-010: contentSource = rss_description_degraded is distinct from rss_description_fallback', () => {
  ...
  assert.ok(SUMMARY_SRC.includes("'rss_description_degraded'"),
